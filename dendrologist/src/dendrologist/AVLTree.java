...
/*===> BEGIN: Augmented public methods <===*/   
   ...
   public E getParent(E entry) throws AVLTreeException
   {
	   if (root == null) {
	    ...
	    while (cur != null) 
	    {
	        int cmpResult = cmp.compare(entry, cur.data);
	        if (cmpResult == 0) 
	        {
	            // The specified entry is found in the current node.
	            // Return the data in the parent node (if it exists).
	            if (parentNode != null) 
	            {
   ...
   public int descendants(E entry) throws AVLTreeException
   {
	   Node curNode = root;
	      while (curNode != null) 
	      {
	          int cmpResult = cmp.compare(entry, curNode.data);
	             
	          if(cmpResult == 0) 
	          {
	        	  return countDesc(curNode);
	          } 
	          else if (cmpResult < 0) 
	          {
...
/* BEGIN: Augmented Private Auxiliary Methods */
    ...
    private int countDesc(Node node)
    {
        ...
        {
        	return 2+countDesc(node.right)+countDesc(node.left);
        }
    }    
    ...
    private boolean isComplete(Node node, int index)
    {
    	 if (node == null) {
    	    ...
    	    while (!queue.isEmpty()) {
    	        ...
    	        if (current != null && current.bal == BalancedFactor.EH) {
    	            if (index < count) {
