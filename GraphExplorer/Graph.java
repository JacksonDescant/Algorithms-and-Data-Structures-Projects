...
/***------------------  BEGIN AUGEMENTED METHOD ----------------***/
   ...
   {
      Vertex fromVertex = first;
      while (fromVertex != null && cmp.compare(fromKey, fromVertex.data) > 0)
         fromVertex = fromVertex.pNextVertex;

      // If 'fromVertex' does not exist or does not match 'fromKey', there is no edge
      if (fromVertex == null || cmp.compare(fromKey, fromVertex.data) != 0)
         return false;

      // Now search for an edge to 'toKey'
      Edge currentEdge = fromVertex.pEdge;
      while (currentEdge != null) {
         if (cmp.compare(currentEdge.destination.data, toKey) == 0)
   ...
   public long countEdges()
   {
      ...
      while (currentVertex != null) {
         Edge currentEdge = currentVertex.pEdge;
         // Count all edges in the current vertex's edge list
         while (currentEdge != null) {
            edgeCount++;
            currentEdge = currentEdge.pNextEdge;
         }
         currentVertex = currentVertex.pNextVertex;
      }
